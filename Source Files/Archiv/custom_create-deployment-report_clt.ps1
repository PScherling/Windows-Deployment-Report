...
.NOTES
          FileName: custom_create-deployment-report_clt.ps1
...
.Example
	...
	.\custom_create-deployment-report_clt.ps1
#>
...
if([string]::IsNullOrEmpty($global:OSDisplayVersion) -and $global:OSVersion -eq "10.0.19044" -and $global:WindowsVersion -eq "2009") { $global:OSDisplayVersion = "21H2" }
...
"@
    ...
    else <#if ($WinRMSvc.Status -ne 'Running')#> {
...
"@
    ...
    if ($SNMPFeature.State -eq 'Installed') {
        #Write-Host $SNMPFeature.DisplayName "is installed." -ForegroundColor Green | &#9989
        #Write-Host $SNMPFeature.Description
		
		<#Add-Content $global:FilePath -Value @"
        <tr>
        <td>Implemented Setting:</td>
        <td>$SNMPName is installed.</td>
        </tr>
"@
#>
		Add-Content $global:FilePath -Value @"
...
"@
        ...
        #Write-Host "SNMP is not installed." -ForegroundColor Red | &#10060
		
		<#Add-Content $global:FilePath -Value @"
        <tr>
        <td>Implemented Setting:</td>
        <td>SNMP is not installed.</td> 
        </tr>
"@
#>
...
"@
	...
	if ($tls1_3ClientValue2.DisabledByDefault -ne "0") {
		Write-Log "TLS 1.3 Client is disabled by default."
		Add-Content $global:FilePath -Value @"
		<tr>
        <td>Implemented Setting:</td>
...
"@
    ...
    elseif ($tls1_3ClientValue2.DisabledByDefault -eq "0") {
		Write-Log "TLS 1.3 Client is not disabled by default."
		Add-Content $global:FilePath -Value @"
		<tr>
        <td>Implemented Setting:</td>
...
"@
	...
	foreach($SMB1Feature in $smbv1Status){
...
"@
		...
		} elseif ($smbv1Status.State -eq "Disabled") {
...
"@
			...
			Write-Log "ERROR: Cannotfetch $($SMB1Feature.FeatureName) information.."
			Add-Content $global:FilePath -Value @"
			<tr>
			<td>Implemented Setting:</td>
...
<html>
	<head>
		<title>Deployment Report for<br>$global:Hostname</title>
		<meta charset="utf-8">
...
"@
	Write-Progress -id 1 -Activity "Generating Deployment Report" -Status "Checking Active Services:" -PercentComplete 92
	Write-Host "########################################"
